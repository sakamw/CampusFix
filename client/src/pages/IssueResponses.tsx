...
export default function IssueResponses() {
        ...
        setIssues((prevIssues) =>
          prevIssues.map((issue) =>
            issue.id === issueId
              ? {
                  ...issue,
                  comments: [...(issue.comments || []), response.data],
                }
              : issue,
          ),
        );
  ...
  const handleMarkResolved = async (issueId: number) => {
    try {
      const response = await issuesApi.updateIssue(issueId, {
        status: "resolved",
      });
      if (response.data) {
                  ...
                  <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div
                              ...
                              style={{
                                width: `${issue.progress_percentage || 0}%`,
                              }}
                            ></div>
                          </div>
                          <div className="flex justify-between text-xs text-muted-foreground">
                            <span>
                              Progress: {issue.progress_percentage || 0}%
                            </span>
                            <span>
                              Status: {issue.progress_status || "Not started"}
                            </span>
                          </div>
                        </div>
                            ...
                            <div className="space-y-2">
                              ...
                              {issue.progress_updates.length > 2 && (
                                <Button
                                  ...
                                  className="text-xs"
                                >
                                  View all {issue.progress_updates.length}{" "}
                                  updates
                                </Button>
                              )}
                            </div>
                    ...
                    {issue.status !== "open" && (
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-full ${statusConfig[issue.status].variant === "success" ? "bg-green-100" : "bg-yellow-100"}`}
                        >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Issue is currently{" "}
                            {statusConfig[issue.status].label.toLowerCase()}
                          </p>
                        </div>
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="font-medium">
                            Admin Comments ({getAdminComments(issue).length})
                          </p>
                        </div>
                        {getAdminComments(issue)
                          .slice(0, 2)
                          .map((comment) => (
                            <div
                              key={comment.id}
                    ...
                    {/* Evidence Files */}
                    {issue.evidence_files &&
                      issue.evidence_files.length > 0 && (
                        <div className="space-y-3">
                          <div className="flex items-center gap-2">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setChatOpen(chatOpen === issue.id ? null : issue.id)
                      }
                    >
                    ...
                    <div className="space-y-4">
                      ...
                      {(issue.status === "resolved" ||
                        ...
                        <Alert>
                          ...
                          <AlertDescription>
                            This issue has been marked as {issue.status}. Thank
